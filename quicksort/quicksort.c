...
static int lomuto(void *array, int left, int right, size_t elem_sz,
    ...
    for(int x=left; x<right; x++) {
        if(comp((arr+(x*elem_sz)), (arr+(right*elem_sz))) < 0) {
            swap((arr+(left*elem_sz)), (arr+(x*elem_sz)), elem_sz);
            left++;
        }
    }
    swap((arr+(left*elem_sz)), (arr+(right*elem_sz)), elem_sz);
